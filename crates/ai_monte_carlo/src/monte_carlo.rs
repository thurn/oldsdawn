...
impl<TState: GameStateNode> StateEvaluator<TState> for RandomPlayoutEvaluator {
    fn evaluate(&self, input: &TState, player: TState::PlayerName) -> Result<i32> {
        let mut game = input.make_copy();
        loop {
            match game.status() {
                GameStatus::Completed { winner } => {
                    return Ok(if winner == player { 1 } else { -1 });
                }
...
pub struct MonteCarloAlgorithm {}

impl SelectionAlgorithm for MonteCarloAlgorithm {
    ...
    {
        run_search(|i| i % 100 == 0 && deadline < Instant::now(), node, evaluator, player)
    }
}
...
/// been explored. The UCT algorithm provides a mathematical heuristic
...
/// ```
pub fn run_search<TStateNode, TEvaluator>(
    ...
    TEvaluator: StateEvaluator<TStateNode>,
{
    ...
    while !should_halt(i) {
        ...
        backup(&mut graph, player, node, reward)?;
        i += 1;
    }

    let (action, _) = best_child(&graph, root, node.legal_actions()?.collect(), 0.0)?;
    Ok(action)
}
...
fn best_child<TState: GameStateNode>(
...
) -> Result<(TState::Action, NodeIndex)> {
    ...
    let (edge, _) = graph
        ...
        .map(|edge| {
            ...
            (edge, exploration + (exploration_bias * exploitation))
        })
        .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
        .with_error(|| "No children found")?;
    Ok((edge.weight().action, edge.target()))
}
